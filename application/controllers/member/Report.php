...
class Report extends CI_Controller {
    ...
    public function laporan_infus()
    {
        ...
        $this->load->view('member/report/laporan_infus',$data);
        $this->load->view('member/template/footer');
    }
    public function json_stok_infus()
    {
        $get_data = $this->Main_model->getSelectedData('stok_infus a', 'a.*,b.fullname', array('a.created_by'=>$this->session->userdata('id')), '', '', '', '', array(
        ...
        foreach ($get_data as $key => $value) {
            ...
            if($value->status=='1'){
                ...
                $href_ubah_data = 'href="'.site_url('member_side/ubah_data_stok_infus/'.md5($value->id_stok_infus)).'"';
                $href_hapus_data = 'onclick="'.$return_on_click.'" href="'.site_url('member_side/hapus_data_stok_infus/'.md5($value->id_stok_infus)).'"';
            }
            ...
            $isi['action'] =	'
                                <div class="btn-group" style="text-align: center;">
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        <li>
                                            <a href="'.site_url('member_side/detail_data_stok_infus/'.md5($value->id_stok_infus)).'">
    ...
    public function tambah_laporan_infus()
    {
        $data['parent'] = 'report';
        $data['child'] = 'infus';
        $data['grand_child'] = '';
        ...
        $this->load->view('member/template/header',$data);
        $this->load->view('member/report/tambah_laporan_infus_1',$data);
        $this->load->view('member/template/footer');
    }
    ...
    public function tambah_laporan_infus_2()
    {
        ...
        $this->load->view('member/template/header',$data);
        $this->load->view('member/report/tambah_laporan_infus_2',$data);
        $this->load->view('member/template/footer');
    }
    public function tambah_barang_infus()
    {
        ...
        if($this->db->trans_status() === false){
            ...
            echo "<script>window.location='".base_url()."member_side/tambah_laporan_infus/'</script>";
        }
        else{
    ...
    public function simpan_data_laporan_infus()
    {
        ...
        if($isi==NULL){
            $this->session->set_flashdata('gagal','<div class="alert alert-warning"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Oops! </strong>barang tidak boleh kosong.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/tambah_laporan_infus/'</script>";
        }else{
            $this->db->trans_start();
            ...
            foreach ($isi as $key => $value) {
                ...
                $this->Main_model->insertData('stok_infus_request',$data_insert1b);
            ...
            if($this->db->trans_status() === false){
                ...
                $this->session->set_flashdata('sukses','<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Yeah! </strong>data telah berhasil ditambahkan.<br /></div>' );
                echo "<script>window.location='".base_url()."member_side/laporan_infus/'</script>";
            }
        }
    }
    public function detail_data_stok_infus()
    {
        ...
        $data['data_utama'] = $this->Main_model->getSelectedData('stok_infus a', 'a.*,f.fullname', array('md5(a.id_stok_infus)'=>$this->uri->segment(3)), '', '', '', '', array(
            ...
            'on' => 'a.verificator=f.user_id',
            'pos' => 'LEFT'
        ))->result();
        $this->load->view('member/template/header',$data);
        $this->load->view('member/report/detail_data_stok_infus',$data);
        $this->load->view('member/template/footer');
    }
    public function ubah_data_stok_infus()
    {
        ...
        $this->load->view('member/template/header',$data);
        $this->load->view('member/report/ubah_data_stok_infus',$data);
        $this->load->view('member/template/footer');
    }
    public function perbarui_data_stok_infus()
    {
        ...
        if($this->db->trans_status() === false){
            ...
            echo "<script>window.location='".base_url()."member_side/ubah_data_stok_infus/".$this->input->post('id')."'</script>";
        }
        else{
    ...
    public function perbarui_data_barang_request_stok_infus()
    {
        $this->db->trans_start();
        $data_insert1 = array(
            'qty' => $this->input->post('qty'),
        ...
        // print_r($data_insert1);
        $this->Main_model->updateData('stok_infus_request',$data_insert1,array('md5(id_stok_infus_request)'=>$this->input->post('id')));
        ...
        if($this->db->trans_status() === false){
            $this->session->set_flashdata('gagal','<div class="alert alert-warning"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Oops! </strong>data gagal diperbarui.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/detail_data_stok_infus/".$this->input->post('id')."'</script>";
        }
        else{
            $this->session->set_flashdata('sukses','<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Yeah! </strong>data telah berhasil diperbarui.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/detail_data_stok_infus/".$this->input->post('id')."'</script>";
    ...
    public function hapus_data_stok_infus_barang()
    {
        ...
        $qty = 0;
        // $totalharga = 0;
        $datadetail = $this->Main_model->getSelectedData('stok_infus_request a', 'a.*', array('md5(a.id_stok_infus_request)'=>$this->uri->segment(3)))->result();
        ...
        $this->Main_model->deleteData('stok_infus_request', array('md5(id_stok_infus_request)'=>$this->uri->segment(3)));
        ...
        if($this->db->trans_status() === false){
            $this->session->set_flashdata('gagal','<div class="alert alert-warning"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Oops! </strong>data gagal dihapus.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/detail_data_stok_infus/".md5($id)."'</script>";
        }
        else{
            $this->session->set_flashdata('sukses','<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Yeah! </strong>data telah berhasil dihapus.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/detail_data_stok_infus/".md5($id)."'</script>";
    ...
    public function laporan_opname()
    {
        ...
        $data['grand_child'] = '';
        $this->load->view('member/template/header',$data);
        $this->load->view('member/report/laporan_opname',$data);
        $this->load->view('member/template/footer');
    }
        ...
        foreach ($get_data as $key => $value) {
            ...
            $isi['action'] =	'
                                <div class="btn-group" style="text-align: center;">
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        <li>
                                            <a href="'.site_url('member_side/detail_data_stok_opname/'.md5($value->id_stok_opname)).'">
                                                <i class="icon-eye"></i> Detil Data </a>
                                        </li>
                                        <li>
                                            <a '.$href_hapus_data.'>
    ...
    public function tambah_laporan_opname_1()
    {
        ...
        $data['barang'] = $this->Main_model->getSelectedData('barang a', 'a.*', array('a.deleted'=>'0'))->result();
        $this->load->view('member/template/header',$data);
        $this->load->view('member/report/tambah_laporan_opname_1',$data);
        $this->load->view('member/template/footer');
    }
    ...
    public function simpan_data_laporan_opname()
    {
        ...
        if($this->db->trans_status() === false){
            $this->session->set_flashdata('gagal','<div class="alert alert-warning"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Oops! </strong>data gagal ditambahkan.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/laporan_opname/'</script>";
        }
        else{
            $this->session->set_flashdata('sukses','<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Yeah! </strong>data telah berhasil ditambahkan.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/laporan_opname/'</script>";
    ...
    public function detail_data_stok_opname()
    {
        ...
        $this->load->view('member/template/header',$data);
        $this->load->view('member/report/detail_data_stok_opname',$data);
        $this->load->view('member/template/footer');
    }
    public function ubah_data_stok_opname()
    {
        ...
        $this->load->view('member/template/header',$data);
        $this->load->view('member/report/ubah_data_stok_opname',$data);
        $this->load->view('member/template/footer');
    }
    public function perbarui_data_stok_opname()
    {
        ...
        if($this->db->trans_status() === false){
            $this->session->set_flashdata('gagal','<div class="alert alert-warning"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Oops! </strong>data gagal diperbarui.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/ubah_data_stok_opname/".$this->input->post('id')."'</script>";
        }
        else{
            $this->session->set_flashdata('sukses','<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Yeah! </strong>data telah berhasil diperbarui.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/laporan_opname/'</script>";
    ...
    public function perbarui_status_data_stok_opname()
    {
        ...
        if($this->db->trans_status() === false){
            $this->session->set_flashdata('gagal','<div class="alert alert-warning"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Oops! </strong>data gagal diperbarui.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/detail_data_stok_opname/".$this->input->post('id')."'</script>";
        }
        else{
            $this->session->set_flashdata('sukses','<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Yeah! </strong>data telah berhasil diperbarui.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/detail_data_stok_opname/".$this->input->post('id')."'</script>";
    ...
    public function hapus_data_stok_opname_barang()
    {
        ...
        if($this->db->trans_status() === false){
            $this->session->set_flashdata('gagal','<div class="alert alert-warning"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Oops! </strong>data gagal dihapus.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/detail_data_stok_opname/".md5($id)."'</script>";
        }
        else{
            $this->session->set_flashdata('sukses','<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong></i>Yeah! </strong>data telah berhasil dihapus.<br /></div>' );
            echo "<script>window.location='".base_url()."member_side/detail_data_stok_opname/".md5($id)."'</script>";
    ...
    {
        if($this->input->post('modul')=='modul_ubah_data_barang_request_stok_infus'){
            $data = $this->Main_model->getSelectedData('stok_infus_request a', 'a.*,b.nama_barang', array('md5(a.id_stok_infus_request)'=>$this->input->post('id')), '', '', '', '', array(
                'table' => 'barang b',
                'on' => 'a.id_barang=b.id_barang',
                'pos' => 'LEFT'
            ))->row();
            ...
            echo'
            <form role="form" class="form-horizontal" action="'.base_url('member_side/perbarui_data_barang_request_stok_infus').'" method="post">
                ...
                <div class="form-body">
                    ...
                    <div class="form-group form-md-line-input has-danger">
                        <label class="col-md-2 control-label" for="form_control_1">Jumlah Barang <span class="required"> * </span></label>
                        <div class="col-md-10">
                            <div class="input-icon">
                                <input type="number" name="qty" class="form-control" value="'.$data->qty.'" required min="1" max="999999">
                                <div class="form-control-focus"> </div>
            ...
            $pecah_tanggal = explode(' ',$data->created_at);
            echo'
            <form role="form" class="form-horizontal" action="'.base_url('member_side/perbarui_status_data_stok_opname').'" method="post">
                <input type="hidden" name="'.$this->security->get_csrf_token_name().'" value="'.$this->security->get_csrf_hash().'">
                <input type="hidden" name="id" value="'.$this->input->post('id').'" readonly>
                <div class="form-body">
                    <div class="form-group form-md-line-input has-danger">
