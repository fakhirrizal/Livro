...
class App extends CI_Controller {
	...
	public function launcher()
	{
		...
		$this->load->view('spv/app/launcher');
	...
	public function menu()
	{
		...
		// $data['data_jenis'] = $this->Main_model->manualQuery($q6);
		$this->load->view('spv/template/header',$data);
		$this->load->view('spv/app/menu',$data);
		$this->load->view('spv/template/footer');
	}
	public function log_activity()
	{
		...
		$this->load->view('spv/template/header',$data);
		$this->load->view('spv/app/log_activity',$data);
		$this->load->view('spv/template/footer');
	}
	public function cleaning_log(){
		...
		if($this->db->trans_status() === false){
			...
			echo "<script>window.location='".base_url()."spv_side/log_activity/'</script>";
		}
		else{
	...
	public function ajax_function(){
		if($this->input->post('modul')=='modul_detail_log_aktifitas'){
			...
			$this->load->view('spv/app/ajax_detail_log_aktifitas',$data);
		}
	}
}