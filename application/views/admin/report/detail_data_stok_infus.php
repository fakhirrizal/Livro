...
<div class="page-content-inner">
	<div class="m-heading-1 border-yellow m-bordered" style="background-color:#FAD405;">
		...
		<p> 1. Jika permintaan "<b>disetujui</b>" maka akan dialihkan ke halaman selanjutnya untuk pengisian item yang diminta (jika itu request dari <b>Bar & Kitchen</b>).</p>
	</div>
	<div class="row">
		<div class="col-md-12">
			<div class="portlet light ">
				<div class="portlet-body">
						...
						if(isset($data_utama)){
							foreach($data_utama as $row)
							{
								...
								if($row->approval==NULL){
									...
									$data_verificator = $this->Main_model->getSelectedData('user_profile a', 'a.*', array('a.user_id'=>$row->verificator))->row();
									$approval = '&nbsp;&nbsp;'.$this->Main_model->convert_tanggal($explode_tgl[0]).' '.substr($explode_tgl[1],0,5).'&nbsp;&nbsp;oleh '.$data_verificator->fullname;
								}
					...
					<div class='row'>
											...
											<table class="table table-striped table-bordered" id="tbl1">
												...
												<tbody>
													...
													foreach ($data_detil as $key => $value) {
														...
														// $td_aksi = '';
														// if($st=='0'){
														// 	$btn_del = '<a onclick="'.$return_on_click.'" class="btn btn-xs red" href="'.site_url('admin_side/hapus_data_stok_infus_barang/'.md5($value->id_stok_infus_detail)).'">
											...
											<table class="table table-striped table-bordered" id="tbl1">
												...
												<tbody>
													...
													foreach ($data_detail_laporan as $key => $value) {
														...
														$return_on_click = "return confirm('Anda yakin?')";
														if($st=='0'){
															$btn_del = '<a onclick="'.$return_on_click.'" class="btn btn-xs red" href="'.site_url('admin_side/hapus_data_stok_infus_barang/'.md5($value->id_stok_infus_request)).'">
															Hapus Data <i class="fa fa-trash"></i> </a>';
														}else{
															$btn_del = '<a class="btn btn-xs red" href="#" disabled>
															Hapus Data <i class="fa fa-trash"></i> </a>';
						...
						<div class="col-md-12" >
						<hr><a href="<?php echo base_url()."admin_side/laporan_infus"; ?>" class="btn btn-info" role="button"><i class="fa fa-angle-double-left"></i> Kembali</a></div>
					</div>
				</div>
			</div>
		</div>
